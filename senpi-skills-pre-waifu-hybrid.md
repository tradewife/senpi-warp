...
license: Apache-2.0
compatibility: "Node.js, shell, Python 3, OpenClaw (optional â€” Step 5 cron)"
metadata:
  author: Senpi
  version: "1.2.2"
  platform: senpi
...
|---|---|---|
| `SENPI_MCP_ENDPOINT` | `https://mcp.prod.senpi.ai` | `https://mcp.prod.senpi.ai` |
| `REFERRAL_CODE` | `` | _(empty â€” optional)_ |
...
**CRITICAL: Every word you output goes directly to the user.** There is no internal channel. Execute all steps silently â€” no narration, no status updates, no reasoning. The ONLY text you may produce is the final welcome message in Step 2.

**Between every tool call: zero text output. Not a single word.**

**Forbidden â€” never write any of these:**
...
If you find yourself about to write anything â€” stop. Execute the next step silently instead.
...
- **If `SENPI_AUTH_TOKEN` is set and (optionally) MCP responds:** Skip **only Step 1** (Onboard). You **MUST** still run **Step 2: Welcome** (show the welcome message and present the three options), then **Step 3: Guide**, then **Step 4: Expand**. Do not treat "token is set" as "onboarding complete â€” do nothing."
- **If `SENPI_AUTH_TOKEN` is unset or empty:** Proceed to **Step 1: Onboard** as usual.

After the initial check, follow the step flow: Step 1 (unless skipped) â†’ **Step 2 (always)** â†’ Step 3 â†’ Step 4.
...
**Gate â€” verify before proceeding:**
...
Do not add balance or funding text â€” you do not have balance data yet; Step 2.5 fetches it and surfaces the appropriate funding message. Present the full welcome template (including the three options and the Agents Arena line) and wait for the user to respond.
...
**Gate â€” verify before proceeding:** User has seen the welcome message and chosen a path.

---

## Step 2.5: Check Funding Status

**Run this step always**, after Step 2 and before Step 3. Call `account_get_portfolio` to fetch the user's current balance. Do this silently â€” do not narrate the tool call.
...
(`npx skills add ... -g`) â€” skip this step silently. If `openclaw` is not
...
This is **explicit-ask only** â€” do not auto-insert this summary into normal
onboarding steps.
