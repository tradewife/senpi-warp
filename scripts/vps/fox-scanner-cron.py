...
def try_auto_entry(sig, strategies, config):
    ...
    if "error" not in res:
        ...
        attach_position_playbook(
            dsl, scanner="fox", margin=margin, leverage=lev,
            score=sig["score"], reasons=sig["reasons"]
        )
        sfile = get_strategy_state_dir(target_strat["_key"]) / f"dsl-{sig['token']}.json"
        save_json(sfile, dsl)
        
        log(f"FOX: Auto-entered {sig['mode']} {sig['direction']} {sig['token']} @ ${eprice:.2f}")
        send_telegram(f"🦊 FOX ENTRY: {sig['mode']} {sig['direction']} {sig['token']}\nScore: {sig['score']}\nMargin: ${margin:.0f}")
        ...
        record_trade({
            ...
            "entrySource": f"fox-{sig['mode'].lower()}", "entryMode": sig['mode']
        })
        ...
        for sig in comb:
            if is_auto_entry_enabled():
                ...
                add_pending_entry({
                    ...
                    "autoEntered": False, "score": sig["score"], "source": "fox",
                    "mode": sig["mode"]
                })


if __name__ == "__main__":
    if not acquire_lock("fox-scanner"): sys.exit(0)
    try:
