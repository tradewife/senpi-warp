#!/usr/bin/env python3
"""
SUGURU — Elite-tier Hyperliquid USDC-margined perpetuals trader.
...
def check_preconditions() -> dict:
    if not os.environ.get("SENPI_AUTH_TOKEN") and not os.environ.get("SENPI_API_KEY"):
        raise RuntimeError("No Senpi auth token - aborting SUGURU")
    ...
    if risk_mode == "RISK_OFF" and not (DRY_RUN or SCAN_ONLY or EXECUTE_APPROVED):
        ...
        log("SUGURU: RISK_OFF bypassed (scan/dry-run mode)")
        risk_mode = "BASELINE"  # Use BASELINE params for scan
    elif risk_mode == "RISK_OFF" and EXECUTE_APPROVED:
        log("SUGURU: RISK_OFF — executing hermes-approved trades only")
...
def execute_trade(
    ...
    if DRY_RUN:
        log(f"SUGURU DRY-RUN: would queue {direction} {asset} "
    ...
    write_journal_row(
        {
            ...
            "provenance_tags": ["suguru", "alo", f"gss-{trade['entryScore']}"],
        }
    )
...
def check_stale_suguru_orders():
    now = datetime.now(UTC)
    now_aest = datetime.now(AEST)
    cancelled = []

    for dsl_file in STATE_DIR.rglob("dsl-*-suguru.json"):
        try:
            ...
            if reason:
                ...
                dsl["closeReason"] = f"suguru_stale:{reason}"
...
def execute_approved():
    """Execute trades approved by hermes decision layer (suguru-approved.json)."""
    approved_file = OUTPUTS_DIR / "suguru-approved.json"
    data = load_json(approved_file)
    if not data:
        log("SUGURU: no suguru-approved.json found — nothing to execute")
        return

    trades = data.get("approved", [])
    if not trades:
        log(f"SUGURU: 0 approved trades ({data.get('summary', 'no reason')})")
        return

    strategies = get_enabled_strategies()
    if not strategies:
        log("SUGURU: no enabled strategies")
        return
    ...
    for trade in trades:
        ...
        if not all(k in trade for k in ("asset", "direction", "leverage", "margin_usd")):
            log(f"SUGURU: skipping {trade.get('asset', '?')} — missing fields")
            continue
        # Remap to execute_trade format
        exec_trade = {
            "asset": trade["asset"],
            "direction": trade["direction"],
            "price": trade.get("entry_price", 0),
            "stopPrice": trade.get("stop_price", 0),
            "tp1Price": trade.get("tp1_price", 0),
            "tp2Price": trade.get("tp2_price", 0),
            "leverage": trade["leverage"],
            "marginUsd": trade["margin_usd"],
            "entryScore": trade.get("entryScore", trade.get("gss", 0)),
            "netRr": trade.get("netRr", 0),
            "atr": trade.get("atr", 0),
            "qty": trade.get("qty", 0),
        }
        result = execute_trade(exec_trade, strategy_id, strategy_key, kg_triples)
        if result:
            executed.append(trade["asset"])
            if trade.get("hermes_reason"):
                log(f"SUGURU: hermes said: {trade['hermes_reason']}")

    if kg_triples:
        append_graph_triples(kg_triples)

    # Clear approved file after execution
    save_json(approved_file, {"executed": True, "timestamp": now_iso()})
    log(f"SUGURU: executed {len(executed)} hermes-approved trades: {', '.join(executed)}")
    git_sync(f"auto: suguru execute-approved ({len(executed)} trades)")


def main():
    if not acquire_lock("suguru"):
        return
    ...
    try:
        ...
        if available_slots <= 0:
            send_telegram("?? SUGURU: no free slots.")
            return
        ...
        # Build full trade proposals for ALL candidates
        strategies = get_enabled_strategies()
        if not strategies:
            log("SUGURU: no enabled strategies")
            return
        ...
        save_json(OUTPUTS_DIR / "suguru-candidates.json", scan_output)
        log(f"SUGURU: {len(candidates_out)} candidates scored")

        # --scan-only: write candidates and exit (hermes reads this file)
        if SCAN_ONLY:
            log("SUGURU: scan-only mode — candidates written, exiting")
            return

        # --dry-run: show what we'd trade, don't execute
        if DRY_RUN:
            for c in candidates_out[:available_slots]:
                log(f"SUGURU DRY-RUN: {c['direction']} {c['asset']} "
                    f"gss={c['gss']:.2f} px={c['entry_price']} "
                    f"lev={c['leverage']}x margin=${c['margin_usd']}")
            return

        trades = []
        for c in candidates_out[:available_slots]:
            # Reconstruct trade dict for execute_trade
            trade = {
                "asset": c["asset"],
                "direction": c["direction"],
                "price": c["entry_price"],
                "stopPrice": c["stop_price"],
                "tp1Price": c["tp1_price"],
                "tp2Price": c["tp2_price"],
                "leverage": c["leverage"],
                "marginUsd": c["margin_usd"],
                "entryScore": c["gss"],
                "netRr": c["net_rr"],
                "atr": c["atr"],
                "qty": c["notional"] / c["entry_price"] if c["entry_price"] > 0 else 0,
            }
        ...
        top_gss = scored[0]["gss"] if scored else 0
        output = {
            "run_id": run_id,
            "timestamp_aest": datetime.now(AEST).isoformat(),
            "trending_universe": universe,
            "candidates_scored": len(candidates_out),
        ...
        save_json(OUTPUTS_DIR / "suguru-state.json", output)
        log(f"SUGURU: {len(candidates_out)} candidates | {len(trades)} trades | GSS top={top_gss:.2f}")

        git_sync("auto: suguru run")
...
if __name__ == "__main__":
    if "--stale" in sys.argv:
        check_stale_suguru_orders()
    elif "--execute-approved" in sys.argv:
        execute_approved()
    else:
        main()
