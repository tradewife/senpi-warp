...
def build_thesis(coin, config):
    ...
    price = (
        float(candles_15m[-1].get("close", candles_15m[-1].get("c", 0)))
        if candles_15m
        else 0
    )
    ...
    reasons = [
        f"4h_{trend_4h.lower()}_{trend_strength:.0%}",
        f"1h_confirms_{mom_1h:+.2f}%",
    ]
    ...
    elif (direction == "LONG" and funding > 0.01) or (
...
def scan():
    ...
    for pos in bison_positions:
        valid, reasons = evaluate_held_position(
            pos["asset"], pos["direction"], config.get("entry", {})
        )
        if not valid:
    ...
    if not signals:
        log(
            f"BISON: Scanned top {len(candidates)} volume assets, no conviction thesis >= {min_score}"
        )
        return
    ...
    if best["score"] >= 12:
        margin_pct = base_pct * 1.5
    elif best["score"] >= 10:
        margin_pct = base_pct * 1.25
    else:
        margin_pct = base_pct
    ...
    log(f"BISON: Signal {asset} {dirn} score={best['score']}")
...
def main():
    if not acquire_lock("bison-scanner"):
        return
    try:
        record_heartbeat("bison")
        scan()
    finally:
        release_lock("bison-scanner")
