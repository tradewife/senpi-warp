...
from senpi_common import (
    ...
    is_entries_allowed, is_auto_entry_enabled, is_rotation_cooled_down,
    POSITION_STATE_DIR, CONFIG_DIR, record_trade, add_pending_entry,
    record_heartbeat, get_all_open_positions
)

MANTIS_CONFIG_FILE = CONFIG_DIR / "mantis-config.json"
SCAN_HISTORY_FILE = POSITION_STATE_DIR / "mantis-scan-history.json"
TRADE_JOURNAL_FILE = Path(__file__).resolve().parent.parent / "memory" / "trade-journal.json"
...
def fetch_markets():
    ...
    if isinstance(raw, dict): raw = raw.get("markets", [])
    return raw
...
def parse_scan(raw_markets):
    ...
    for i, m in enumerate(raw_markets[:TOP_N]):
        ...
        scan["markets"].append({
            ...
            "contribution": round(float(m.get("pct_of_top_traders_gain", m.get("longPct", 0))), 6),
            "traders": int(m.get("trader_count", m.get("traders", 0))),
            "price_chg_4h": round(float(m.get("token_price_change_pct_4h", 0) or 0), 4),
        })
    return scan
...
def check_asset_volume(token, dex=""):
    ...
    res = mcporter_call("market_get_asset_data", {"asset": asset, "candle_intervals": ["1h"], "include_funding": False})
    if "error" in res: return 0, False
    d = res.get("data", res)
    candles = d.get("candles", {}).get("1h", []) if isinstance(d, dict) else []
    if len(candles) < 6: return 0, False
...
def is_erratic_history(rank_history, exclude_last=False):
    ...
    if exclude_last and len(nums) > 1: nums = nums[:-1]
    if len(nums) < 3: return False
    for i in range(1, len(nums) - 1):
        prev = nums[i] - nums[i - 1]
        nxt = nums[i + 1] - nums[i]
        if prev < 0 and nxt > ERRATIC_REVERSAL_THRESHOLD: return True
        if prev > 0 and nxt < -ERRATIC_REVERSAL_THRESHOLD: return True
    return False
...
def time_of_day_modifier():
    ...
    if 4 <= hour < 14: return 1, "time_bonus_optimal_window"
    if hour >= 18 or hour < 2: return -2, "time_penalty_chop_zone"
    return 0, None


def check_4h_alignment(direction, price_chg_4h):
    if direction == "LONG" and price_chg_4h < 0: return False
    if direction == "SHORT" and price_chg_4h > 0: return False
    return True
...
def detect_stalker_signals(current_scan, history, stalker_cfg):
    ...
    if len(prev) < min_consecutive: return []
    ...
    for m in current_scan["markets"]:
        ...
        if len(valid_r) < min_consecutive + 1: continue
        
        recent_r = [r for _, r in valid_r[-(min_consecutive + 1):]]
        is_climbing = all(recent_r[i] >= recent_r[i + 1] for i in range(len(recent_r) - 1))
        tot_climb = recent_r[0] - recent_r[-1]
        
        if not is_climbing or tot_climb < min_total_climb: continue
        if is_erratic_history(rhist, exclude_last=True): continue
        ...
        if require_vol and not vol_build: continue
        ...
        score += tmod
        if treas: reasons.append(treas)
        
        if score >= min_score:
            signals.append({
                "token": token, "direction": direction, "mode": "STALKER",
                "score": score, "reasons": reasons, "rank": rank
            })
...
def detect_striker_signals(current_scan, history, striker_cfg):
    ...
    for m in current_scan["markets"]:
        ...
        if not is_fj and not is_imm: continue
        
        rcontribs = []
        for s in prev[-5:]:
            sm = get_market_in_scan(s, token, dex)
            if sm: rcontribs.append(sm["contribution"])
        ...
        avel = abs(cvel)
        if jump < min_jump and avel < min_vel_ovr: continue
        
        if avel < min_vel_floor and not (is_fj and cvel > 0): continue
        ...
        if score < min_score or len(reasons) < min_reasons: continue
        ...
        signals.append({
            "token": token, "direction": dirn, "mode": "STRIKER",
            "score": score, "reasons": reasons, "rank": rank
        })
...
def build_dsl_state(sig, config, price):
    ...
    return {
        "active": True, "asset": sig["token"], "direction": sig["direction"],
        "mode": sig["mode"], "score": score, "phase": 1,
        "highWaterPrice": price, "highWaterRoe": 0, "currentTierIndex": -1,
        "consecutiveBreaches": 0, "lockMode": "pct_of_high_water",
        "phase2TriggerRoe": 7,
        "phase1": {
            "enabled": True, "retraceThreshold": 0.03,
        ...
        "tiers": DSL_TIERS, "stagnationTp": STAGNATION_TP, "convictionTiers": CONVICTION_TIERS,
        "createdAt": now_iso()
    }


def try_auto_entry(sig, strategies, config):
    if len(get_all_open_positions()) >= MAX_POSITIONS: return
    ...
    res = mcporter_call("strategy_create_position", {
        "strategyId": target_strat.get("strategyId"), "asset": sig["token"],
        "direction": sig["direction"], "marginUsd": margin, "leverage": lev,
        "orderType": "MARKET"
    })
    ...
    if "error" not in res:
        ...
        attach_position_playbook(
            dsl, scanner="mantis", margin=margin, leverage=lev,
            score=sig["score"], reasons=sig["reasons"]
        )
        sfile = get_strategy_state_dir(target_strat["_key"]) / f"dsl-{sig['token']}.json"
        save_json(sfile, dsl)
        
        log(f"MANTIS: Auto-entered {sig['mode']} {sig['direction']} {sig['token']} @ ${eprice:.2f}")
        send_telegram(f"🦗 MANTIS ENTRY: {sig['mode']} {sig['direction']} {sig['token']}\nScore: {sig['score']}\nMargin: ${margin:.0f}")
        
        record_trade({
            "action": "OPEN", "asset": sig["token"], "direction": sig["direction"],
            "entryPrice": eprice, "size": float(res.get("size", 0)),
            "margin": margin, "leverage": lev, "strategyKey": target_strat["_key"],
            "entrySource": f"mantis-{sig['mode'].lower()}", "entryMode": sig['mode']
        })
...
def run():
    ...
    if len(hist["scans"]) > 20: hist["scans"] = hist["scans"][-20:]
    ...
    if is_entries_allowed() and comb:
        ...
        for sig in comb:
            if is_auto_entry_enabled():
                ...
                add_pending_entry({
                    "asset": sig["token"], "direction": sig["direction"],
                    "autoEntered": False, "score": sig["score"], "source": "mantis",
                    "mode": sig["mode"]
                })


if __name__ == "__main__":
    if not acquire_lock("mantis-scanner"): sys.exit(0)
    try:
