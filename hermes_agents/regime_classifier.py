#!/usr/bin/env python3
"""
Hermes Regime Classifier - Runs every hour
...
def main():
    ...
    if current_regime.get("riskMode") != new_risk_mode:
        print(f"[Regime Classifier] Regime change: {current_regime.get('riskMode')} -> {new_risk_mode}")
        
        new_regime = {
            "riskMode": new_risk_mode,
            "updatedAt": datetime.utcnow().isoformat() + "Z",
            "updatedBy": "hermes-regime",
            "reason": reason
        }
        ...
        os.system(f'git commit -m "regime classifier: {new_risk_mode} - {reason}" > /dev/null 2>&1')
